...
function formatDate(value: Publisher['created_at']) {
...
export default function ShowPublisher({ record }: { record: Publisher }) {
    const navigate = useNavigate();
    const { toast } = useToast();
    const queryClient = useQueryClient();

    const deleteMutation = useMutation({
        mutationFn: () => http.delete(`${publisherConfig.endpoint.replace('/api/', '/')}/${record.id}`),
        onSuccess: async () => {
    ...
    return (
        <AppLayout breadcrumbs={[...publisherConfig.breadcrumbs, { title: `ID: ${record.id}`, href: '#' }]}>
            <Head title={`Publisher #${record.id}`} />
            <div className="flex h-full flex-1 flex-col gap-4 overflow-auto p-4 md:p-6">
                <div className="flex items-start justify-between gap-4">
                    <h1 className="text-2xl font-semibold">Publisher Details</h1>
                    <div className="flex items-center gap-2">
                        ...
                        <Button
                            ...
                            onClick={() => {
                                if (confirm(`Delete publisher #${record.id}?`)) deleteMutation.mutate();
                            }}
                        >
                        ...
                        <Button asChild className="gap-2 bg-blue-600 text-white shadow-sm hover:bg-blue-700 focus-visible:ring-blue-500">
                            <Link href={`${publisherConfig.basePath}/${record.id}/edit`}>
                ...
                <div className="rounded-lg border bg-card p-6 shadow-sm">
                    ...
                    <div className="grid gap-x-12 gap-y-8 md:grid-cols-2 xl:grid-cols-3">
                        ...
                        <DetailItem label="Established year">{record.established_year}</DetailItem>
                        ...
                        <div className="space-y-1 md:col-span-2 xl:col-span-3">
                            ...
                            <p className="whitespace-pre-wrap text-sm font-medium">{record.address || <span className="italic text-muted-foreground">Not available</span>}</p>
                        </div>
                    </div>
                </div>
            </div>

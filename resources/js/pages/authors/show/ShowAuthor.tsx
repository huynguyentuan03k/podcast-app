...
function formatDate(value: Author['created_at']) {
...
export default function ShowAuthor({ record }: { record: Author }) {
    const navigate = useNavigate();
    const { toast } = useToast();
    const queryClient = useQueryClient();

    const deleteMutation = useMutation({
        mutationFn: () => http.delete(`${authorConfig.endpoint.replace('/api/', '/')}/${record.id}`),
        onSuccess: async () => {
            toast({ title: 'delete author successfully', description: 'author has been deleted.' });
            await queryClient.invalidateQueries({ queryKey: ['authors'] });
            navigate(authorConfig.basePath);
        },
        onError: () => {
            toast({ title: 'delete author failed', description: 'Something went wrong', variant: 'destructive' });
        },
    });

    return (
        <AppLayout breadcrumbs={[...authorConfig.breadcrumbs, { title: `ID: ${record.id}`, href: '#' }]}>
            <Head title={`Author #${record.id}`} />
            <div className="flex h-full flex-1 flex-col gap-4 overflow-auto p-4 md:p-6">
                <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button
                            ...
                            onClick={() => {
                                if (confirm(`Delete author #${record.id}?`)) deleteMutation.mutate();
                            }}
                        >
                        ...
                        <Button asChild className="gap-2 bg-blue-600 text-white shadow-sm hover:bg-blue-700 focus-visible:ring-blue-500">
                            <Link href={`${authorConfig.basePath}/${record.id}/edit`}>
                ...
                <div className="rounded-lg border bg-card p-6 shadow-sm">
                    ...
                    <div className="grid gap-x-12 gap-y-8 md:grid-cols-2 xl:grid-cols-3">
                        ...
                        <DetailItem label="Avatar">
                            <Avatar className="size-20">
                                <AvatarImage src={record.avatar_url ?? undefined} />
                                <AvatarFallback>{record.name?.slice(0, 1).toUpperCase() ?? 'A'}</AvatarFallback>
                            </Avatar>
                        </DetailItem>
                        <DetailItem label="Email">{record.email}</DetailItem>
                        <DetailItem label="Website">
                            {record.website ? (
                                <a className="text-blue-600 hover:underline" href={record.website} target="_blank" rel="noreferrer">
                                    {record.website}
                                </a>
                            ) : null}
                        </DetailItem>
                        ...
                        <div className="space-y-1 md:col-span-2 xl:col-span-3">
                            ...
                            <p className="whitespace-pre-wrap text-sm font-medium">{record.bio || <span className="italic text-muted-foreground">Not available</span>}</p>
                        </div>
                    </div>
                </div>
            </div>

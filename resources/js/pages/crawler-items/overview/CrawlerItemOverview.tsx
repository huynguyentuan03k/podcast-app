...
export default function CrawlerItemOverview() {
    ...
    const columns = useMemo(
        () => getCrawlerItemColumns({ crawlingId, deletingId, onCrawl: setCrawlTarget, onRequestDelete: setDeleteTarget }),
        [crawlingId, deletingId],
    );
    ...
    return (
        <AppLayout breadcrumbs={crawlerItemConfig.breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-auto p-4 md:p-6">
                <DataTableV1
                    title={crawlerItemConfig.title}
                    columns={columns}
                    queryKey={['crawler-items']}
                    ...
                    actions={({ selectedRows, table }) => {
                        ...
                        return selectedItems.length > 0 ? (
                            ...
                            <Button asChild size="sm" className="gap-2 bg-blue-600 text-white hover:bg-blue-700">
                                <Link href={`${crawlerItemConfig.basePath}/create`}>
                    ...
                    queryFn={async (request) => {
                        const response = await http.get(buildCrawlerItemIndexUrl(request));
